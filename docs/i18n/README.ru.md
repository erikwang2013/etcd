...
- PHP >= 8.1
- сервер etcd v3.x
- HTTP-клиент PSR-18 + PSR-17 (обязателен для HTTP-транспорта, во фреймворках обычно уже есть)
...
$etcd = new EtcdClient([
    ...
    'timeout'   => 5.0,     // секунды (задаётся в клиенте PSR-18)
...
|------|------|------|---------|
| **HTTP** | готов | PSR-18 + PSR-17 | без расширений, доступен сразу |
