...
`auto` равнозначен `http`: `GrpcTransport` всё ещё скелет (все три метода бросают исключение), поэтому **не** переключается на gRPC сам — реальная проверка `ext-grpc` только лишила бы работы тех, у кого расширение установлено. Выбрать его можно лишь явно: `'transport' => 'grpc'`. Когда gRPC появится, смысл здесь изменится.
...
│   ├── Support/KeyValue.php         # общее декодирование: чтение KV и события watch дают одну и ту же форму
