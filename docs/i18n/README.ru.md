...
$etcd = new EtcdClient([
    ...
    'auth'      => [        // необязательно; для обмена учётных данных на token нужен https
        'user'     => 'root',
        'password' => 'secret',
    ],
]);
...
$etcd->kv()->get('/start', [
    ...
    'minModRevision'    => 100,       // только key, изменённые с этой ревизии
    'maxModRevision'    => 200,
    'minCreateRevision' => 100,       // фильтр по ревизии создания
...
// вложенная транзакция: в ветке может быть ещё одна транзакция
$etcd->kv()->txn(
    compare: [['result' => 0, 'target' => 3, 'key' => '/lock', 'value' => 'free']],
    success: [[
        'request_put' => ['key' => '/lock', 'value' => 'mine'],
        'request_txn' => [                       // внутренняя транзакция
...
**Остановка наблюдения:** `watch()` блокируется навсегда — передайте ему `WatchHandle`, и его можно остановить извне (обычная потребность долгоживущего процесса, завершающегося по SIGTERM):
...
После `cancel()` `watch()` **возвращает управление нормально** (исключения нет, catch не нужен). Даже на простаивающем key он выходит примерно за секунду — драйвер curl замечает это по периодическому колбэку cURL, драйвер stream — по циклу простоя с таймаутом чтения 200 мс.

**Переподключение:** при обрыве соединения Watch переподписывается с `lastRevision + 1` (`start_revision` — **включающая** граница, поэтому возобновление с прежнего значения проиграло бы последнее событие). При отказе узла события не теряются и не дублируются.

**Политика повторов:** повторяются только сбои, доказывающие, что соединение вообще не было установлено (отказ в соединении / сбой DNS), а RPC только для чтения (range, status, memberlist и т. п.) дополнительно терпят 5xx и таймауты. Запись при 5xx или таймауте чтения **не** повторяется — запрос мог уже примениться, а повтор применит CAS дважды или даже даст «уверенно неверный ответ» (повтор увидит результат своей первой записи и сообщит, что CAS не прошёл, хотя на самом деле он выиграл).
...
**Как происходит аутентификация:** etcd v3 не принимает HTTP Basic — он требует сначала обменять учётные данные на token (`POST /v3/auth/authenticate`), а затем отправлять token как есть: `Authorization: <token>` (префикс `Bearer` тоже отклоняется). Если заданы `auth.user` / `auth.password`, клиент делает это **автоматически** и кэширует token, а при 401 один раз аутентифицируется заново — вызывать вручную не нужно. Можно обменять и самому:

```php
$token = $etcd->auth()->authenticate('root', 'secret');  // полученный token переиспользуется в последующих запросах
```

Для отправки учётных данных нужен `scheme => 'https'`: при обычном http конструктор сразу откажет (чтобы пароль не уходил в открытом виде).
...
### Election — выборы лидера

```php
// участие: сначала берём аренду, потом выдвигаемся; возврат только после победы, проигравшие ждут (ограничение — $timeout)
$lease  = $etcd->lease()->grant(30);
$leader = $etcd->election()->campaign('/my-election', 'node-a', $lease['ID'], 5.0);
// → ['name' => ..., 'key' => ..., 'rev' => ..., 'lease' => ...]

// текущий leader (null, если никто не избран)
...
На HTTP-шлюзе `campaign()` — это **буферизованный ответ**: он не вернётся до победы, поэтому ограничивайте ожидание через `$timeout`. Для длительного отслеживания смены leader используйте `observe()`.

**Внимание (измеренная тихая ловушка):** `proclaim()` / `resign()` требуют **полного массива-описания leader** (name, key и rev — все три). Если чего-то не хватает, etcd вернёт **HTTP 200 и не сделает ничего** — выглядит как успешное освобождение, а leader на месте. Поэтому оба метода проверяют описание перед отправкой и бросают исключение при неполном. Всегда используйте значения, возвращённые `campaign()` / `leader()`, и не собирайте их вручную.

**Прочее измеренное поведение:** кампания, сорвавшаяся в середине запроса, **отзывается** сервером (поэтому таймаут `acquire()` может безопасно сообщить о неудаче, не превращаясь в «скрытого держателя»); когда никто не избран, `leader()` возвращает `null` (сервер отвечает 500 `election: no leader` — это нормальное состояние, а не ошибка).
...
**Это клиентская реализация поверх Election, а не серверная блокировка.** HTTP-шлюз etcd 3.5 **не предоставляет** `/v3/lock/*` (измерено: 404), поэтому взаимное исключение обеспечивают «кампания Election + аренда» — тот же подход, что в Go-пакете `concurrency` самого etcd. Если держателя убили через `SIGKILL`, блокировка снимается автоматически по истечении аренды, без ручной очистки.
...
// для большой БД — потоковая запись на диск: не читать всю базу в память
$bytes = $etcd->maintenance()->snapshotTo('/backup/etcd-snapshot.db');
// сначала пишется временный файл, а переименование — только после проверки sha256-дайджеста
// в последних 32 байтах, поэтому прерванная запись не оставит файл, похожий на бэкап; возвращает число записанных байт.
```
...
| **gRPC** | унарные RPC | ext-grpc + grpc/grpc + сгенерированные protobuf-сообщения | высокая скорость; стриминг и Election — только через HTTP |
| **auto** | по умолчанию | — | сейчас совпадает с `http` (см. ниже) |

`auto` равнозначен `http`: gRPC сейчас покрывает только унарные RPC — потоковые вызовы вроде watch / snapshot и Election по-прежнему идут через HTTP, поэтому автоматическое переключение молча лишило бы половины возможностей тех, у кого расширение установлено. Выбрать его можно лишь явно: `'transport' => 'grpc'`.

**Текущее состояние gRPC-транспорта (обратите внимание):**
- **Реализовано**: унарные RPC (`send()`) — классы сообщений сгенерированы `protoc` из `rpc.proto` etcd v3.5; тело запроса собирается из типизированных сообщений, а имена и типы полей гарантирует proto.
- **Не реализовано**: потоковые вызовы вроде watch и snapshot, а также `/v3/election/*` (это другой proto, `v3electionpb`) — такие пути **отклоняются по имени** с объяснением причины, без молчаливого отказа.
- **Нет сквозной проверки**: в среде разработки этого проекта нет `ext-grpc`, поэтому открытие канала, metadata с учётными данными, `_simpleRequest`, таймауты и отображение кодов состояния **написаны вручную по образцу вывода grpc_php_plugin и ни разу не проверялись реальным вызовом**. Генерация сообщений и сборка запросов покрыты тестами, сетевой обмен — нет. Если собираетесь использовать gRPC, проверьте всё сами перед продакшеном.
...
├── src/
│   ├── EtcdClient.php               # верхний фасад + синглтон: доступ к восьми подсистемам
...
│   │   ├── GrpcTransport.php        #   транспорт gRPC (унарные RPC; стриминг отклоняется с пояснением)
│   │   └── GrpcStub.php             #   подкласс Grpc\BaseStub (отдельный файл, не загружается без расширения)
...
│   ├── Election/                    # Election — выборы (campaign / leader / observe / resign)
│   ├── Lock/LockClient.php          # Lock — распределённая блокировка (поверх Election; в шлюзе нет /v3/lock/*)
│   ├── Maintenance/                 # обслуживание Maintenance: status / alarm / defrag / snapshot
│   ├── Exception/                   # иерархия исключений
│   ├── Support/                     # общее декодирование KeyValue / Int64, дескриптор отмены WatchHandle
...
```bash
composer install
vendor/bin/phpunit --no-coverage tests/Unit     # модульные тесты (стабы в памяти)

# интеграционные: у набора есть собственный шлюз-двойник, повторяющий настоящий (chunked-разбиение + конверт {"result":…} + int64 строками)
php -d zend.assertions=1 -d assert.exception=1 tests/transport_test.php
```

**Проверка на реальном кластере (рекомендуется):** дайте тому же набору адрес живого etcd — он выполнит те же случаи и проверит, что двойник и настоящий шлюз совпадают (разбиение, конверт, кодирование int64):
...
> Зачем это стоит делать: в проекте когда-то жил набор дефектов (watch не работал вовсе, девять методов
> без аргументов отдавали 400, keepAlive всегда бросал исключение) при 40+ зелёных тестах — двойник и
> настоящий шлюз различались **и в разбиении, и в конверте**. Их исправили, и этот режим не даёт классу ошибок вернуться.

Согласованность документации и схем тоже держат скрипты:

```bash
python3 scripts/i18n/check_translations.py      # 13 переводов: переключатель, ссылки, каталог, структура блоков
python3 scripts/i18n/build_diagrams.py --check --all   # схемы на 13 языках: каждая строка измерена
python3 scripts/i18n/sync_zh_readme.py --check  # копия zh синхронна с корневым README
```

CI (`.github/workflows/ci.yml`) запускает всё перечисленное: модульные тесты на PHP 8.2/8.3, синтаксический порог на PHP 8.0/8.1, интеграционный набор (включая дифференциальный прогон против настоящего etcd в сервис-контейнере) и три проверки документации.
