...
> **注意（2026-09-25 补注）：** 本报告写于 2026-08-02，其"所有 P0-P3 已修复"的结论**已被推翻**。
> 后续一次面向真实 etcd 的复核发现六项 P0（watch 完全不可用、9 个无参方法 400、
> keepAlive 必抛、snapshot 产出无效备份、认证整体失效、重试腐蚀 CAS），
> 根因是测试桩与真实网关在分帧与信封两方面都不一致，导致 40+ 测试全绿而缺陷存活。
> 详见 README 的传输模式/认证/重试章节与 `tests/transport_test.php` 的回归用例。
> 本文件保留为当时的历史记录。
