...
T = {
    ...
    "arch.http.badge": "可用",
    "arch.grpc.sub1": "生成的消息类 + Grpc\\Channel",
    "arch.grpc.sub2": "ext-grpc + grpc/grpc + protobuf 运行时",
    "arch.grpc.badge": "一元 RPC",
    ...
    "arch.msg.title": "Support 层 · 共用解码",
    "arch.msg.body": "网关把 int64/uint64 发成 JSON 字符串、零值直接省略，字节字段是 base64。解码只此一处，KV 读取与 watch 事件因此类型一致。",
    "arch.msg.i1": "KeyValue::decode() 统一解码",
    "arch.msg.i2": "int64 → int，uint64 溢出留字符串",
    "arch.msg.i3": "watch 事件与 get() 同形",
    "arch.dep.title": "运行依赖",
    "arch.dep.1": "psr/http-client ^1.0  可选",
    "arch.dep.2": "psr/http-factory ^1.0  可选",
